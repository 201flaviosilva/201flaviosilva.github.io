...
export function generatePlayerParticles(scene, player, x = 0, y = 0) {
  ...
  const moveEmitter2A = SQUARES_PARTICLES.createEmitter({
    ...
    frame: range(0, 9),
    moveToY: {
      min: scene.scale.height / 2 - 150,
      max: scene.scale.height / 2 + 150,
    },
...
// Player Shoot
export function generatePlayerShootParticles(
  scene,
  shoot,
  sprite,
  x = 0,
  ...
  return {
    moveParticles,
    moveParticlesEmitter,
    explosionParticles,
    explosionParticlesEmitter,
  };
}
...
export function generateEnemyParticles(scene, enemy, x = 0, y = 0) {
  ...
  const particlesEmitter = particles.createEmitter({
    ...
    scale: { start: 0.5, end: 0 },
    alpha: { start: 0.5, end: 0 },
    rotate: { start: -90, end: randomNumber(-360, 360) },
    lifespan: { min: 100, max: 500 },
  });
