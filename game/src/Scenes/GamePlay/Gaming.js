...
export default class Gaming extends Phaser.Scene {
  ...
  create() {
    ...
    // Triggers
    const triggers = [
      this.add.zone(0, middleHeight),
      this.add.zone(width, middleHeight),
    ];
      ...
      (enemy, shoot) => {
        if (enemy.type === shoot.type) {
          ...
          if (this.enemiesGroup.countActive() === 0)
            this.enemiesGroup.generateAllEnemies();
        }
      },
      null,
      this
    ...
    EventSystem.on(EVENTS_NAMES.restartGame, () => {
      this.scene.restart();
    });
  }
}
