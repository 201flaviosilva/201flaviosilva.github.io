  ...
  create() {
    ...
    this.physics.add.overlap(
      ...
      (player, enemy) => {
        ...
        player.hitted();
        enemy.kill();
        EventSystem.emit(EVENTS_NAMES.scoreCollision, -1, {
          x: enemy.x,
          y: enemy.y,
        });
      }
    ...
    this.physics.add.overlap(
      this.player,
      this.enemiesShootsGroup,
      (player, enemyShoot) => {
        // Player vs EnemyShoot
        player.hitted();
        enemyShoot.kill();
        EventSystem.emit(EVENTS_NAMES.scoreCollision, -1, {
          x: enemyShoot.x,
    ...
    this.physics.add.overlap(
      ...
      this.enemiesGroup,
      (playerShoot, enemy) => {
        // PlayerShoot vs Enemy
        playerShoot.kill();
        enemy.kill();
        EventSystem.emit(EVENTS_NAMES.scoreCollision, 10, {
          x: playerShoot.x,
          y: playerShoot.y,
        });
      }
    ...
    this.physics.add.overlap(
      ...
      (playerShoot, enemyShoot) => {
        // PlayerShoot vs EnemyShoot
        playerShoot.particlesExplode();
        enemyShoot.kill();
        EventSystem.emit(EVENTS_NAMES.scoreCollision, 100, {
          x: playerShoot.x,
          y: playerShoot.y,
        });
      }
...
export class WebUI extends Phaser.Scene {
  ...
  create() {
    ...
    this.scoreLabel = this.add
      .text(this.scale.width / 2, 50, this.score, TextStyle.web.scoreLabel)
      .setOrigin(0.5);
...
class UpdateScoreLabel extends Phaser.GameObjects.Text {
  constructor(
    ...
    y = 10,
    score = 0,
    style = TextStyle.web.updateScoreLabel
  ) {
