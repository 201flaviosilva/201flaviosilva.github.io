import { Maths, randomNumber } from "utilidades";
...
export default class SnakeAll extends Phaser.Scene {
  ...
  create() {
    ...
    EventSystem.on(EVENTS_NAMES.restartGame, (value) => {
      this.scene.restart();
    });
  }
  ...
  newApple() {
    ...
    if (
      // Check if there is an apple or the snake body at the same position of the new apple
      this.apples.find(
        (apple) =>
          apple.x === x * snakeTileSize && apple.y === y * snakeTileSize
      ) ||
      this.player.body
        .getChildren()
        .find(
          (body) => body.x === x * snakeTileSize && body.y === y * snakeTileSize
        )
    )
      return;
    ...
    const apple = new Apple(
      ...
      x,
      y,
      APPLES_NAMES[Phaser.Math.Between(0, APPLES_NAMES.length - 1)]
    );
  ...
  update(time) {
    ...
    this.apples.forEach((apple) => {
      if (
        this.player.head.x === apple.x &&
        this.player.head.y === apple.y &&
        apple.isAlive
      ) {
        if (apple.isWall) {
...
export class SnakeAllUI extends Phaser.Scene {
  ...
  create() {
    ...
    this.scoreLabel = this.add
      .text(this.scale.width / 2, 50, this.score, TextStyle.snake.scoreLabel)
      .setOrigin(0.5);
    ...
    this.restartButton = new Button(this, {
      ...
      upCallback: () => {
        EventSystem.emit(EVENTS_NAMES.restartGame);
      },
    ...
    // Events
    EventSystem.on(EVENTS_NAMES.gameOver, (value) =>
      this.restartButton.changeVisibility(true)
    );

    EventSystem.removeListener(EVENTS_NAMES.increaseScore);
    EventSystem.on(
      EVENTS_NAMES.increaseScore,
      this.createNewScoreLabel.bind(this)
    );
